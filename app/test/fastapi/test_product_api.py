...
from app.test import (
    create_mock_product_repository,
...
class TestUserApi:
    ...
    def test_get_product_catalog(self):
        with self.app.container.product_repository.override(
            create_mock_product_repository()
        ):
    ...
    def test_post_register_product(self):
        with self.app.container.product_repository.override(
            ...
            response = self.client.post(
                self.base_path, json=expected_product_description
            )
            assert expected_product_description == response.json()
            assert response.status_code == 200
    ...
    def test_put_update_product(self):
        with self.app.container.product_repository.override(
            ...
            response = self.client.put(
                self.base_path + "/mock_id", json=expected_product_description
            )
            assert expected_product_description == response.json()
            assert response.status_code == 200
