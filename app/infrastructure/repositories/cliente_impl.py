...
class ClienteRepositoryImpl(ClienteRepository):
    ...
    async def get_all(self) -> list[ClienteEntity] | None:
    ...
    async def get_by_cpf(self, cpf: str) -> ClienteEntity | None:
        ...
        async for session in self.database.get_session():
            stmt = select(ClienteModel).where(
                (ClienteModel.cpf == cpf) | (ClienteModel.cpf == cpf_clean)
            )
            result = await session.execute(stmt)
            cliente = result.scalar_one_or_none()
    ...
    async def add(self, cliente: ClienteEntity) -> ClienteEntity | None:
        async for session in self.database.get_session():
    ...
    async def update(self, cliente: ClienteEntity) -> ClienteEntity | None:
        try:
