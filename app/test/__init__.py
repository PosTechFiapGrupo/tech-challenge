...
from app.test.data.product import (
    expected_products_catalog,
...
def create_mock_product_repository(mock_repository: Mock):
    ...
    mock_product_repo.get_all = Mock(
        return_value=[
            ProductEntityFactory.create(**product)
            for product in product_repository_get_all
        ]
    )
    mock_product_repo.get_by_id = Mock(
        return_value=ProductEntityFactory.create(**product_repository_get_by_id)
    )
    mock_product_repo.add = Mock(
        return_value=ProductEntityFactory.create(**product_repository_get_by_id)
    )
    mock_product_repo.update = Mock(
        return_value=ProductEntityFactory.create(**product_repository_get_by_id)
    )
